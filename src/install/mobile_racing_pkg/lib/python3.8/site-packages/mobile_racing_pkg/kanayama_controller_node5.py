...
class KanayamaControllerNode(Node):
    def __init__(self):
        ...
        # ===== Lookahead 파라미터 (타입 안전 버전) =====
        # launch/yaml에서 5 또는 5.0이 들어와도 안 깨지도록 double로 선언
        self.declare_parameter('use_lookahead', True)
        self.declare_parameter('lookahead_base', 5.0)   # points (double)
        self.declare_parameter('lookahead_gain', 0.3)   # points per (m/s)
        ...
        self.use_lookahead = self.get_parameter('use_lookahead').get_parameter_value().bool_value
        self.lookahead_base = self.get_parameter('lookahead_base').get_parameter_value().double_value
        ...
        self.get_logger().info(
            f'KanayamaControllerNode initialized (safe lookahead). '
            f'use_lookahead={self.use_lookahead}, '
            f'base={self.lookahead_base}, gain={self.lookahead_gain}'
        )
    ...
    def ego_callback(self, msg: Float32MultiArray):
        ...
        if self.use_lookahead:
            ...
            if look < 0:
                look = 0
            idx_ref = idx + look
            if idx_ref >= self.num_points:
                idx_ref = self.num_points - 1
        else:
            idx_ref = idx
